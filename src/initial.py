...
def defineDate():
    ...
    try:
        with open(file_path, "w") as f:
            f.write("Carga inicial realizada em: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    except Exception as e:
        logger.error(f"Erro ao criar o arquivo initial_load.txt: {e}")
        sys.exit(1)
...
def flowMongo():
    ...
    if not is_valid_cnpj(cnpj_loja):
        logger.error("Erro: O CNPJ da loja deve ter exatamente 14 dígitos e não pode conter pontuacao.")
        sys.exit(1)
    ...
    try:
        logger.info("Tentando conexao com instancia MongoDB...")
...
def main():
   # Verifica se o arquivo já existe
    if os.path.exists(file_path):
        logger.info("O arquivo initial_load.txt já existe. O script não será executado.")
        sys.exit(0)  # Termina a execução do script
    else:
        flowMongo()
